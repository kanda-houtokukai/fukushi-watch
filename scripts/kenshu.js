...
/* ===========================================================================
 * 中央福祉学院（kenshu-gakuin・P35で実装・⚠️**P53で見送り**＝台帳の状態は「見送り」）:
 * 「募集中の講座」カテゴリ（/info-cate/course/）の【募集開始】【申込開始】型の記事。
 * タイトル・本文に締切（「第一次締切 9/24」「申込は7/24まで」）と研修日程が平文。
 *
 * ⚠️**このパーサは動くが、台帳が「見送り」なので呼ばれない。消さずに残してある。**
 * 見送りの理由（2026-08-28の再調査・[DECISION]）:
 *   ①**オンライン／地方開催が0件**——24講座すべて神奈川（ロフォス湘南）への宿泊・通学が前提。
 *     福岡からは受講料（中央値4万円台）に交通費と3〜5泊の宿泊費が上乗せされる。
 *   ②**「募集中の講座」14件のうち実際の募集告知は6件**で、残りは受講者向けのお知らせ。
 *     巡回すると申し込めない情報が半分以上混ざる（P51で解消した状態を自分から作りに行く）。
 *   ③**直リンクが規約上原則不可**で、承諾が得られない可能性が相応にある。
 * ⚠️**再開の条件: 学院がオンライン開催（またはブロック開催）を増やしたら再検討する。**
 *   その場合は台帳 #22 の状態を「巡回中」に戻すだけでよい（届出の送付が前提）。
 *   照会文書 `~/Documents/fukushi-watch-taigai/gakuin-link-todoke-2026-08-26.html` も未送付で残してある。
 * ======================================================================== */
