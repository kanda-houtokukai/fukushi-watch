...
export const ZENBUNYA_KW =
  /虐待|ＢＣＰ|BCP|業務継続|感染症|身体拘束|権利擁護|外国人|技能実習|特定技能|育成就労|喀痰吸引/;
// 喀痰吸引: 高齢だけでなく障害（重度訪問介護等）・児童（医療的ケア児）にもまたがる制度
// （P36-2・ユーザー決定）。⚠️語彙は実データで必要になったものだけ足す——机上で増やさない
// （点検済みで見送り: 医療的ケア・看取り・認知症・身元保証・成年後見。認知症の分野またぎ
//   （若年性=障害）はAIが優先順①で正しく複数分野を付けており例外の出番が無い）。
...
行政および福祉関係団体のサイトの新着情報の一覧（タイトル・カテゴリ・日付のみ。本文はありません）から、
各項目について次を判定してください。
...
2. importance: 福祉事業者一般にとっての実務上の重要度を3段階で判定（分野は問わない）
   ...
   - 「低」= 実務への影響が薄い（人事異動・調達情報・採用・行事/イベント・広報・
      福祉と関係のない行政情報）
   - 迷ったら「中」に倒す
   - ⚠️業界団体（全国社会福祉協議会など）の助成・研修の公募案内は原則「中」。
  ...
  for (const batch of batches) {
    ...
    batch.forEach((it, i) => {
      ...
      judgedAll.push({
        hash: it.hash,
        source: it.source,
        // 団体(P21)だけ kind を残す。印章［団］の判定に使う。
        // 行政は既存の履歴と形を揃えるため付けない（差分を無意味に増やさない）
