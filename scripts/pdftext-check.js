 ...
 * ⚠️**合格条件は 15/15 のまま緩めない**（ユーザー判断・2026-08-28）。実測は12/15だが、
 *   差の3件は「AIが但し書きを省略する」既知のもので、規則方式が完全であることの裏付け。
 *   基準を実測に合わせて下げると、精度が落ちたときに検知できなくなる。
...
if (process.argv.includes("--ai")) {
  ...
  // ⚠️合格条件は **15/15 のまま緩めない**（ユーザー判断・2026-08-28）。
  //   現在の差は「AIが但し書き（※…）を省略する」という既知のもので、**規則方式が完全である
  //   ことの裏付け**。ここを実測値に合わせて下げると、将来モデルが変わって精度が落ちたときに
  //   検知できなくなる。基準は動かさず、既知の差との増減を見る。
  // ⚠️AIの出力は実行ごとに揺らぐ（同じ日に走らせて 3件 → 2件 と動いた・実測）。
  //   1回の増減で騒がず、**続けて増えるなら**モデルのずれを疑うこと。
  const KNOWN_DIFF = 3; // 2026-08-28 実測の上振れ側（AIが但し書きを省略した件数）
  const diffs = target - agree;
  console.log(
    `  ※ 既知の差${KNOWN_DIFF}件（AIが但し書きを省略）。これが増えたらモデルのずれを疑う。` +
      `今回の差異は${diffs}件` +
      (diffs > KNOWN_DIFF
        ? " ← ⚠️既知の差より多い。上の差異の中身を読むこと（続けて増えるならモデルのずれ）"
        : diffs < KNOWN_DIFF
          ? " ← 既知の差より少ない（AIの揺らぎの範囲。何度か走らせて安定するなら件数を更新してよい）"
          : " ← 既知の差と同数（想定どおり）")
  );
